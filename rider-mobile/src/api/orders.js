...
import * as SecureStore from 'expo-secure-store'; // R12: receipt URL token
...
// ── R12: Receipt download ─────────────────────────────────────

/**
 * Returns the full URL for the PDF receipt, with ?token= appended.
 * Use with Linking.openURL() — no extra packages needed.
 */
export async function getReceiptUrl(orderId) {
  const token = await SecureStore.getItemAsync('auth_token').catch(() => null);
  const base  = client.defaults.baseURL || 'http://localhost:3000/api/v1';
  return `${base}/customer/orders/${orderId}/receipt${token ? `?token=${encodeURIComponent(token)}` : ''}`;
}
