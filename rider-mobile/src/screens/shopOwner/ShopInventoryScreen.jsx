...
function stockStatus(item) {
  if (!item.is_listed || item.stock_quantity === 0) return 'out';
...
function QuickUpdateModal({ item, visible, onClose, onSave }) {
  ...
  useEffect(() => {
    if (visible) setValue(String(item?.stock_quantity || 0));
  }, [visible, item]);
        ...
        <View style={styles.editSheet}>
          ...
          <View style={styles.editField}>
            ...
            <View style={styles.priceInput}>
              ...
              <TextInput
                ...
                value={String(Math.round(form.price / 100))}
                onChangeText={v => setForm(f => ({ ...f, price: (parseInt(v, 10) || 0) * 100 }))}
                keyboardType="number-pad"
                placeholderTextColor={Colors.textTertiary}
              />
            </View>
          ...
          <View style={styles.editField}>
            ...
            <TextInput
              ...
              onChangeText={v => setForm(f => ({ ...f, stock_quantity: parseInt(v, 10) || 0 }))}
              keyboardType="number-pad"
              placeholderTextColor={Colors.textTertiary}
            />
          </View>
          ...
          <View style={styles.toggleRow}>
            <View>
              ...
              <Text style={styles.toggleSub}>
                {form.is_listed ? 'Customers can order this item' : 'Hidden from customers'}
              </Text>
            </View>
            <Switch
              value={form.is_listed}
              onValueChange={v => setForm(f => ({ ...f, is_listed: v }))}
...
function InventoryCard({ item, onQuickUpdate, onFullEdit }) {
  ...
  return (
    <View style={[styles.invCard, !item.is_listed && styles.invCardDisabled]}>
      <View style={styles.invCardLeft}>
        ...
        {/* Price */}
        <Text style={styles.invPrice}>{fmtPaise(item.price)} / {item.unit || 'unit'}</Text>
        ...
        <Text style={[styles.invStock, { color: style.textColor }]}>
          {!item.is_listed
            ...
            ? '❌ Out of stock'
            : `${style.icon === 'warning' ? '🔴 ' : '✓ '}${item.stock_quantity} ${item.unit || 'units'} in stock`}
        </Text>
      </View>
...
export default function ShopInventoryScreen() {
  ...
  const handleQuickSave = useCallback(async (itemId, stockCount) => {
    ...
    try {
      await updateInventoryItem(itemId, { stock_quantity: stockCount });
      setItems(prev =>
        prev.map(i => i.id === itemId ? { ...i, stock_quantity: stockCount } : i)
      );
    } catch (e) {
      Alert.alert('Error', e.message);
    }
