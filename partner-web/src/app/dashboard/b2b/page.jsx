...
import { api } from '../../../lib/api'; // P20 fix: was 'apiClient' which doesn't exist — correct export is 'api'
...
function ApproveDrawer({ application, onClose }) {
  ...
  const handleApprove = async () => {
    ...
    try {
      await api.patch(`/admin/b2b/applications/${application.id}/approve`, {
...
function RejectDrawer({ application, onClose }) {
  ...
  const handleReject = async () => {
    ...
    setSubmitting(true); setError(null);
    try {
      await api.patch(`/admin/b2b/applications/${application.id}/reject`, { reason });
      qc.invalidateQueries({ queryKey: ['b2b-applications'] });
      onClose();
    } catch (e) {
...
export default function B2BPage() {
  ...
  const { data: appData, isLoading: appLoading } = useQuery({
    ...
    queryFn:  () => tab !== 'overdue'
      ? api.get(`/admin/b2b/applications?status=${tab}`)
      : null,
    enabled:  tab !== 'overdue',
  });
  ...
  const { data: overdueData, isLoading: overdueLoading } = useQuery({
    ...
    queryFn:  () => api.get('/admin/b2b/outstanding'),
    enabled:  tab === 'overdue',
  });
  ...
  const { mutate: markPaid } = useMutation({
    mutationFn: (b2bOrderId) => api.patch(`/admin/b2b/orders/${b2bOrderId}/paid`),
