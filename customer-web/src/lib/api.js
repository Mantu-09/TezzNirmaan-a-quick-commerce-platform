...
export const authApi = {
  ...
  // Phase E: Self-service password change for shop_owner, rider, shop_staff
  changePassword: (newPw)        => api.patch('/auth/change-password', { new_password: newPw }),
};
...
export const inventoryApi = {
  ...
  // Phase 12: Browse master catalog products not yet in shop inventory
  getMasterCatalog:   (params = {}) => api.get(`/shop/catalog${qs(params)}`),
...
export const adminApi = {
  ...
  // Products — Master Catalog (Phase 12)
  getProducts:       (params = {}) => api.get(`/admin/products${qs(params)}`),
  ...
  // Reference data — for product form dropdowns
  getCategories:     ()            => api.get('/admin/categories'),
  getBrands:         ()            => api.get('/admin/brands'),

  // Image upload — admin-scoped R2 presigned URL
  getImageUploadUrl: (folder = 'products', contentType = 'image/jpeg') =>
    api.post('/admin/images/upload-url', { folder, content_type: contentType }),

  // Analytics
  getOverview:    ()            => api.get('/admin/analytics/overview'),

  // Phase E: Staff account creation
  createStaff: (body)          => api.post('/admin/staff', body),
...
// ── Rider Payout Management — P10-5 ───────────────────────────
// GET  /admin/payouts/pending     — list all pending/processing requests
// GET  /admin/payouts/balance     — RazorpayX virtual account balance
// POST /admin/payouts/:id/approve — trigger RazorpayX payout
// POST /admin/payouts/:id/reject  — reject with reason, notify rider
export const payoutsApi = {
  listPending: (params = {}) => api.get(`/admin/payouts/pending${qs(params)}`),
  getBalance:  ()            => api.get('/admin/payouts/balance'),
  approve:     (id)          => api.post(`/admin/payouts/${id}/approve`),
  reject:      (id, reason)  => api.post(`/admin/payouts/${id}/reject`, { reason }),
};

// \u2500\u2500 Investor / Series A Analytics \u2014 P10-6 \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
// GET /admin/analytics/investor       \u2014 all Series A metrics in one call
// GET /admin/analytics/cohorts        \u2014 cohort retention matrix
// GET /admin/analytics/unit-economics \u2014 CAC, LTV, payback period
export const investorApi = {
  getMetrics:      () => api.get('/admin/analytics/investor'),
  getCohorts:      () => api.get('/admin/analytics/cohorts'),
  getUnitEconomics:() => api.get('/admin/analytics/unit-economics'),
  // P11-5: Generate 7-day shareable token
  generateLink:    () => api.post('/admin/investor/generate-link'),
  // P11-6: Add a marketing spend row
  addSpend: (body) => api.post('/admin/investor/spend', body),
};

// ── Phase E: Admin Staff Management ────────────────────────────
export const adminStaffApi = {
  create: (body) => api.post('/admin/staff', body),
};

// ── Phase D: Admin User Management ─────────────────────────────
export const adminUsersApi = {
  list:          (params = {})          => api.get(`/admin/users${qs(params)}`),
  suspend:       (id, reason)           => api.patch(`/admin/users/${id}`, { is_suspended: true,  suspension_reason: reason }),
  reactivate:    (id)                   => api.patch(`/admin/users/${id}`, { is_suspended: false, suspension_reason: null }),
  walletCredit:  (id, amount_paise, reason) => api.post(`/admin/users/${id}/wallet`, { amount_paise, reason }),
};

// ── Phase D: Platform Config ────────────────────────────────────
export const platformConfigApi = {
  get:  ()     => api.get('/admin/platform-config'),
  save: (body) => api.patch('/admin/platform-config', body),
};
