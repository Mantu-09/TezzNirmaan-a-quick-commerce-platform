...
async function generateSitemap() {
  ...
  for (const city of cities) {
    ...
    // City landing page (P10-0: /storefront/city/:slug is now a real route)
    urls.push({
    ...
    // Category pages scoped to this city — uses real app URL pattern
    for (const cat of CATEGORIES) {
      urls.push({
        loc:        `${BASE_URL}/storefront/category/${cat}?city=${slug}`,
        lastmod:    today,
        priority:   '0.7',
        changefreq: 'daily',
      });
  ...
  // Product pages — use city-scoped catalog (P10-0: replaces hardcoded shop slug)
  // Fetch top 48 products from Patna (most established city) for sitemap
  const productsRes = await safeFetch(
    `${API}/api/v1/public/catalog?city=patna&limit=48&sort=popular`
  );
