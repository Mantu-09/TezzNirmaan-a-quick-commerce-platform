...
// ─────────────────────────────────────────────────────────────────────────────
// Session M: Shop Analytics API
// ─────────────────────────────────────────────────────────────────────────────

/**
 * GET /shop/analytics?period=today|7d|30d
 * Returns revenue, orders, topProducts, peakHours, avgDeliveryTime, lowStockAlerts
 * @param {'today'|'7d'|'30d'} period
 */
export async function getShopAnalytics(period = '7d') {
  return client.get(`/shop/analytics?period=${period}`);
}

/**
 * GET /shop/analytics/revenue-trend?days=7|30
 * Returns array of { date, revenue_paise, orders } for sparkline chart
 * @param {7|30} days
 */
export async function getRevenueTrend(days = 7) {
  return client.get(`/shop/analytics/revenue-trend?days=${days}`);
}
