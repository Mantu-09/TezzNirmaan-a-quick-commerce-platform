...
//   • Rider Fleet panel (Phase F) — all riders, live status
...
import useRiderLocations    from '../../../hooks/useRiderLocations'; // Phase F
...
// ── Phase F: Rider Fleet Panel ─────────────────────────────
function RiderFleetPanel() {
  const { riders, loading } = useRiderLocations();

  const onlineRiders   = riders.filter(r => r.is_online);
  const offlineRiders  = riders.filter(r => !r.is_online);
  const delivering     = onlineRiders.filter(r => r.has_active_delivery);

  return (
    <Section title={`Rider Fleet (${riders.length})`} icon="🛵" badge={delivering.length}>
      {loading && riders.length === 0 ? (
        <div style={{ padding: '12px 0', textAlign: 'center', color: TEXT3, fontSize: 12 }}>
          Loading rider data…
        </div>
      ) : riders.length === 0 ? (
        <div style={{ padding: '12px 0', textAlign: 'center', color: TEXT3, fontSize: 12 }}>
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
            {[
              { label: 'Online',     count: onlineRiders.length,   color: '#16a34a', bg: 'rgba(22,163,74,0.1)'  },
              { label: 'Delivering', count: delivering.length,      color: '#f97316', bg: 'rgba(249,115,22,0.1)' },
              { label: 'Offline',    count: offlineRiders.length,   color: TEXT3,     bg: SURFACE2               },
            ].map(c => (
              <div key={c.label} style={{ flex: 1, background: c.bg, borderRadius: 8, padding: '6px 8px', textAlign: 'center' }}>
                <div style={{ fontWeight: 800, fontSize: 16, color: c.color }}>{c.count}</div>
                <div style={{ fontSize: 10, color: TEXT2, marginTop: 1 }}>{c.label}</div>
              </div>
            ))}
          </div>
          ...
          {onlineRiders.map(rider => (
            <div key={rider.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: `1px solid ${BORDER}` }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: rider.has_active_delivery ? '#f97316' : '#16a34a', flexShrink: 0 }} />
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: TEXT }}>{rider.full_name || 'Rider'}</div>
                  <div style={{ fontSize: 11, color: TEXT3 }}>
                    {rider.has_active_delivery ? '📦 Delivering' : '⏳ Waiting'}
                    {rider.updated_at && ` · ${Math.round((Date.now() - new Date(rider.updated_at).getTime()) / 60000)}m ago`}
                  </div>
                </div>
              </div>
              {rider.lat && rider.lng && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  style={{ fontSize: 11, color: PRIMARY, fontWeight: 600, textDecoration: 'none', padding: '4px 8px', background: `${PRIMARY}15`, borderRadius: 6 }}
                >
                  📍 Map
                </a>
              )}
